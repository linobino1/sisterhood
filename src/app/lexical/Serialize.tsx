  ...
  return (
    <Fragment>
      {nodes?.map((node, index): JSX.Element | null => {
        ...
        const thisDisableMarginBottom = disableMarginBlock && index === nodes.length - 1
        if (node.type === 'text') {
          let text = (
        ...
        switch (node.type) {
          ...
          case 'heading': {
            return (
              <Gutter disable={disableGutter} key={index} className={cn(className, 'group')}>
                <Heading
          ...
          case 'list': {
            ...
            return (
              <Gutter as={Tag} disable={disableGutter} key={index} className={node?.listType}>
                {serializedChildren}
              </Gutter>
            )
          }
                  ...
                  {type === 'video' ? (
                    <video
                      ...
                      className={cn('w-full', {
                        ...
                        'fixed left-0 top-0 -z-10 h-screen w-screen object-cover': fullscreen,
                      })}
                    />
                  ) : type === 'image' ? (
